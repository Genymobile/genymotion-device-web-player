...
export default class GPS extends OverlayPlugin {
    ...
    renderWidget() {
        ...
        this.inputComponents.latitude.regexFilter = /^-?\d*\.?\d*$/;
        this.inputComponents.latitude.regexValidField = /^-?(?:[0-8]?\d(?:\.\d*)?|90(?:\.0*)?)$/;

        this.inputComponents.latitude.addEventListener('gm-input', (e) => {
        ...
        this.inputComponents.longitude.regexFilter = /^-?\d*\.?\d*$/;
        this.inputComponents.longitude.regexValidField = /^-?(?:(?:1[0-7]\d(?:\.\d*)?)|(?:[0-9]?\d(?:\.\d*)?)|180(?:\.0*)?)$/;

        this.inputComponents.longitude.addEventListener('gm-input', (e) => {
        ...
        this.inputComponents.altitude.regexFilter = /^-?\d*\.?\d*$/;
        this.inputComponents.altitude.regexValidField = /^-?(?:[0-9]\d{0,3}(?:\.\d*)?|10000(?:\.0*)?)$/;

        this.inputComponents.altitude.addEventListener('gm-input', (e) => {
        ...
        this.inputComponents.accuracy.className = 'gm-gps-accuracy-input';
        this.inputComponents.accuracy.setAttribute('value', '0');
        this.inputComponents.accuracy.setAttribute('unit-text', 'm');
        this.inputComponents.accuracy.regexFilter = /^$|^(0?[0-9]{1,2}|1[0-9]{2}|200)$/;
        this.inputComponents.accuracy.regexValidField = /^$|^(0?[0-9]{1,2}|1[0-9]{2}|200)$/;

        this.inputComponents.accuracy.addEventListener('gm-input', (e) => {
            const value = parseFloat(e.detail.value) || 0;
            this.accuracySlider.value = value;
            this.checkErrors();
        });

        this.inputComponents.accuracy.addEventListener('gm-blur', (e) => {
            if (e.detail.value === '') {
        ...
        if (Object.keys(this.inputComponents).includes('speed')) {
            ...
            this.inputComponents.speed.regexFilter = /^-?\d*\.?\d*$/;
            this.inputComponents.speed.regexValidField = /^(?:[0-9]|[1-9][0-9]|[1-3][0-9]{2})(?:\.\d{0,2})?$/;

            this.inputComponents.speed.addEventListener('gm-input', (e) => {
        ...
        this.inputComponents.bearing.className = 'gm-gps-bearing-input';
        this.inputComponents.bearing.setAttribute('value', '0');
        this.inputComponents.bearing.setAttribute('unit-text', 'o');
        this.inputComponents.bearing.regexFilter = /^$|^(0?[0-9]{1,2}|[1-2][0-9]{2}|3[0-5][0-9]|360)$/;
        this.inputComponents.bearing.regexValidField = /^$|^(0?[0-9]{1,2}|[1-2][0-9]{2}|3[0-5][0-9]|360)$/;

        this.inputComponents.bearing.addEventListener('gm-input', (e) => {
            const value = parseFloat(e.detail.value) || 0;
            this.bearingSlider.value = value;
            this.checkErrors();
        });

        this.inputComponents.bearing.addEventListener('gm-blur', (e) => {
            if (e.detail.value === '') {
