...
class OverlayPlugin {
    ...
    /**
     * Creates a modal template with configurable options
     * @param {Object} [options={}] - Modal configuration options
     * @returns {Object} Modal elements container
     */
        ...
        this.instance.addListener(dragHandle, 'mousedown', (event) => {
            ...
            const removeMouseMoveListener = this.instance.addListener(document, 'mousemove', (e) => {
                ...
                requestAnimationFrame(() => {
                    ...
                    newY = Math.max(0, Math.min(wrapperRect.bottom - modalHeight - wrapperRect.top, newY));
    ...
    calculateModalPosition(triggerElement, toolbarPosition) {
        ...
        switch (toolbarPosition) {
            case 'left':
                x = triggerRect.left + triggerElement.offsetWidth + 10;
                y = triggerRect.top - wrapperRect.top;

                // Adjust for vertical overflow
                if (triggerRect.top + modalHeight > wrapperRect.bottom) {
                    y = Math.max(0, Math.min(wrapperRect.bottom - modalHeight - wrapperRect.top, y)) - 10;
                }
                break;
            case 'right':
            default:
                x = triggerRect.left - modalWidth - 10;
                y = triggerRect.top - wrapperRect.top;
